...
Rectangle.prototype = {
    ...
    // 绘制虚线
    drawDashedLine(ctx, x1, y1, x2, y2, dashLen=4) {
    ...
    buildPath: function (ctx, style) {
        if (!style.radius) {
            if(style.dashed){
                ...
                // 横纵各一条就行了
                this.drawDashedLine(ctx, style.x, style.y, style.x + style.width, style.y)
                this.drawDashedLine(ctx, style.x + style.width, style.y, style.x + style.width, style.y + style.height)
            }else{
                ctx.moveTo(style.x, style.y);
